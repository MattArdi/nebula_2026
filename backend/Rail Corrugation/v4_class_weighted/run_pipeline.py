"""
Rail Corrugation Subsystem (v4, class-weighted ensemble) — Single-Command Pipeline
====================================================================================
...
v4 changes one thing from v3: model.py combines CatBoost/XGBoost/LogReg's
votes with a per-class weight matrix instead of one scalar weight per
model (features.py is untouched -- same 49 features as v3). See
model.py's docstring and algorithm.md Section 5 for the validated numbers
behind this change (macro F1 0.8809 -> 0.8940 in repeated CV, confirmed on
a second, independent set of CV seeds) and why this weighting was chosen
over the alternatives tried (a Side-I-priority matrix, routing Side I
through EasyEnsembleClassifier).
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rail Corrugation subsystem (v4) — class-weighted soft-voting ensemble, one-command run.",
